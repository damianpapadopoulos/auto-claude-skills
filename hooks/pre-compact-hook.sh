...
if command -v jq >/dev/null 2>&1; then
    ...
    TRIGGER="$(printf '%s' "$INPUT" | jq -r '.trigger // "unknown"' 2>/dev/null)" || TRIGGER="unknown"
fi
...
_SESSION_TOKEN=""
# Try singleton first (for single-transcript-per-session semantic)
[ -f "${HOME}/.claude/.skill-session-token" ] && \
    _SESSION_TOKEN="$(cat "${HOME}/.claude/.skill-session-token" 2>/dev/null)"
# Fall back to payload-derived token if singleton empty
if [ -z "$_SESSION_TOKEN" ] && [ -f "${_PLUGIN_ROOT}/hooks/lib/session-token.sh" ]; then
