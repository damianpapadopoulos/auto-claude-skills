...
find src -name "*.ts" -not -path "*/node_modules/*" > /tmp/batch-manifest.txt
echo "Found $(wc -l < /tmp/batch-manifest.txt) files. Review the list:"
cat /tmp/batch-manifest.txt
```
...
head -1 /tmp/batch-manifest.txt | xargs -I{} claude -p "Transform {} as follows: [PROMPT]. Output ONLY the transformed file content." > /tmp/dry-run-output.txt
diff /path/to/original /tmp/dry-run-output.txt
```
...
while IFS= read -r file; do
  if claude -p "Transform $file as follows: [PROMPT]. Output ONLY the file content." > "${file}.tmp" 2>/dev/null; then
    mv "${file}.tmp" "$file"
    echo "OK: $file" >> /tmp/batch-results.log
  else
    rm -f "${file}.tmp"
    echo "FAIL: $file" >> /tmp/batch-results.log
  fi
done < /tmp/batch-manifest.txt
```
...
# Check results
echo "Results:"
grep -c "^OK:" /tmp/batch-results.log
grep -c "^FAIL:" /tmp/batch-results.log

# Retry failures
grep "^FAIL:" /tmp/batch-results.log | cut -d' ' -f2 > /tmp/batch-retry.txt
# Re-run step 3 with /tmp/batch-retry.txt as input
```
...
```bash
split -l 20 /tmp/batch-manifest.txt /tmp/batch-chunk-
for chunk in /tmp/batch-chunk-*; do
...
- **No progress bars** — `wc -l /tmp/batch-results.log` tells you where you are.
