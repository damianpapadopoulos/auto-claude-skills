...
**Acceptance Scenarios:**
- [x] GIVEN a user runs product-discovery and approves a brief with 2 hypotheses WHEN the session completes through SHIP with a local merge — implemented as designed
- [x] GIVEN a user runs the full SDLC loop and ships via PR WHEN outcome-review is invoked — implemented as designed
- [x] GIVEN a user runs through SHIP but picks Option 3 or 4 WHEN the SHIP composition completes THEN no baseline written — implemented as designed
- [x] GIVEN a session that skipped product-discovery WHEN openspec-ship runs THEN hypotheses is null — implemented as designed

**Scope changes:**
- Added: fallback-registry.json restoration fix (corrective commit for accidental session-start hook regeneration) — not in original design but required for correctness
- Added: discovery_path in provenance output — minor additive change consistent with design intent

**Design decision changes:**
- Hypothesis section uses `**Hypotheses:**` bold label (consistent with other brief fields) rather than `## Hypotheses` heading — adapted to match existing product-discovery brief style
