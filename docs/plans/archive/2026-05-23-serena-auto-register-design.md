...
- **Modifying `~/.serena/serena_config.yml`.** The auto-register flow MUST NOT write to the user's global Serena config. The browser-tab suppression is achieved via the per-MCP-server `--open-web-dashboard false` flag, scoped to our registration only. A user who wants tabs back can edit one line in their `~/.claude.json` `mcpServers.serena.args` array, or remove the marker and re-run `/setup` with different choices. Touching `~/.serena/serena_config.yml` would be invasive and would affect Serena instances launched by tools other than Claude Code (Cursor, Cline, Claude Desktop, etc.).
...
This is the canonical command from `commands/setup.md` (line 264) **plus** `--open-web-dashboard false`. The flag is the design's answer to a real UX problem surfaced during implementation: Serena's default behavior is to open a browser tab to its dashboard every time the MCP server starts (i.e., every Claude Code session). For a feature whose stated goal is *zero-touch onboarding*, spamming the user's browser on first session would be exactly the wrong outcome. The flag suppresses the auto-open at the per-MCP-server scope without modifying the user's global Serena config (`~/.serena/serena_config.yml`); the dashboard itself remains enabled and is reachable at `http://localhost:24282/dashboard/` for users who want it.
...
THEN   the plugin runs `claude mcp add --scope user serena -- serena start-mcp-server --context claude-code --project-from-cwd --open-web-dashboard false`
  ...
  AND  no browser tab opens for the Serena dashboard
  AND  `~/.serena/serena_config.yml` is NOT modified
```

**Scenario 1b: Fresh user with default Serena config (web_dashboard_open_on_launch=true)**
```
GIVEN  the user has never set `web_dashboard_open_on_launch: false` in ~/.serena/serena_config.yml (i.e., a brand-new Serena install with defaults)
  AND  Scenario 1's other GIVEN conditions hold
WHEN   the user starts a Claude Code session
THEN   no browser tab opens
  AND  the registered MCP command in ~/.claude.json contains `--open-web-dashboard false` (the flag overrides the config default at per-MCP-server scope)
  AND  the user can still access the dashboard manually at http://localhost:24282/dashboard/
```
...
**Approved approach:** First-time-only auto-registration with user-scope, gated on `command -v serena`, protected by a marker file at `~/.claude/.auto-claude-skills-serena-registered`. Registration command includes `--open-web-dashboard false` to suppress the per-session browser tab without touching the user's global Serena config. `/setup` remains the explicit, richer entry point and the documented recovery path.

**Amendment 2026-05-23:** During implementation it became clear that Serena's default behavior opens a browser tab on every MCP server start. A zero-touch onboarding feature that spams the user's browser on first session is anti-goal. Resolved by passing `--open-web-dashboard false` as part of the registered MCP command (per-MCP-server scope, no global Serena config mutation). The dashboard remains enabled and accessible at `http://localhost:24282/dashboard/`.

**Next:** PLAN phase tasks (helper function, marker file format, session-start integration, `/setup` doc update, tests, changelog) — see `2026-05-23-serena-auto-register-plan.md`.
