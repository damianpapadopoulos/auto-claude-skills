...
- [ ] `test_backfill_excludes_gating_milestones`: fresh state, prompt anchoring
...
- [ ] `test_backfill_ship_prompt_excludes_review`: fresh state, "ship it"-class
      prompt anchoring at `verification-before-completion` (step 5) →
      `.completed` lacks `requesting-code-review`.
- [ ] `test_backfill_preserves_disk_gating_entries`: seed on-disk `.completed`
...
- [ ] Run: all five RED against current walker (fabrication reproduces);
      existing walker tests still green.

## Task 2 — GREEN: filter the computed prefix (hooks/skill-activation-hook.sh)

- [ ] In the state-write block (~:1079-1081), fold the gating-name filter into
...
- [ ] Hardcode the two names adjacent to the existing hardcoded gate invariants
...
- [ ] Fail-open check: jq failure degrades to `_comp_completed="[]"` exactly as
      today (existing `|| _comp_completed="[]"` path covers the filtered
      program too).
- [ ] `/bin/bash -n hooks/skill-activation-hook.sh` (Bash 3.2 syntax gate).
- [ ] Task 1 tests GREEN; full `bash tests/test-routing.sh < /dev/null` green.

## Task 3 — End-to-end deny regression + suite

- [ ] New test in `tests/test-push-gate-failclosed.sh` (or sibling):
...
- [ ] Full suite: `bash tests/run-tests.sh < /dev/null` green.
- [ ] Routing-governance dogfood note for SHIP: this branch touches `hooks/` —
