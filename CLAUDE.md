...
- jq is optional at runtime; session-start falls back to `config/fallback-registry.json`. **Push-gate degradation is no longer silent (audit F5):** session-start runs a precondition canary — missing/unsourceable gate components (`openspec-guard.sh`, `branch-ledger.sh`, `verdict.sh`, `git-command.sh`, `session-token.sh`) emit a `PUSH-GATE CANARY` warning, and the jq-less fallback states the gate falls open. Libs are SOURCE-probed in a stdio-nulled subshell (parse-only `-n` misses the Bash-3.2 expansion-time arithmetic killer — proven by the red fixture); the guard script is parse-checked only (it executes and reads stdin). PAIRED: a new gate-ENFORCEMENT lib must be added to the canary list (advisory-only `consol-marker.sh` is deliberately excluded). Regression: `tests/test-push-gate-canary.sh`. The same five files double as the **installed-plugin drift canary** manifest (post-audit item 1): when a session's cwd is this repo and the plugin runs from the versioned cache, session-start warns `PLUGIN DRIFT CANARY` on manifest-version mismatch or gate-file cksum drift (warning-only, fail-open). A new gate-enforcement lib must be added to BOTH lists in the same change. Regression: `tests/test-plugin-drift-canary.sh`.
