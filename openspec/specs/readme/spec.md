...
Contract for the user-facing README. Decision-funnel ordering, phase-behavior table, example prompts, and explicit boundary statements so first-time visitors can evaluate the plugin in one pass without guessing what it is or what it isn't.
...
Prerequisites (Claude Code CLI, jq) MUST be folded into the Install section rather than appearing as a standalone section.
...
Companion plugins and MCP integrations MUST be grouped by category instead of listed with exact counts.
