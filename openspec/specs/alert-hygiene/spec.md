...
Deterministic triage pipeline for alert-policy noise. Turns raw GCP monitoring signal into prescriptive action categories (Do Now / Investigate / Needs Decision) gated by strict evidence, ownership, and rollback requirements so the report is actionable rather than advisory.
...
#### Scenario: Action-class grouping
- **WHEN** the report renders its main finding sections
- **THEN** top-level groupings are Do Now, Investigate, and Needs Decision — not High/Medium/Low
...
#### Scenario: Systemic section structure
- **WHEN** the report renders systemic-level concerns
- **THEN** a single "Systemic Issues" section contains the four named subsections, and no separate "Label/Scope Inconsistencies" or "Coverage Gaps" top-level sections exist
