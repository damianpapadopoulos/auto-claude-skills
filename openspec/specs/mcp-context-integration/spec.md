...
Bridge between external MCP servers (Forgetful, Serena, PostHog, Atlassian) and the plugin's context stack. Keeps detection flags, tool-name references, and phase-doc guidance aligned with the user's actual MCP surface rather than an assumed one.
