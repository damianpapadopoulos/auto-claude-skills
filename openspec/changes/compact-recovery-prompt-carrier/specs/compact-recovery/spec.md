...
### Scenario 1: auto-compaction recovers at the next prompt
...
### Scenario 2: manual /compact recovery consumes the marker first
...
- GIVEN a confirmed-intent marker and a non-archived OpenSpec change entry exist
...
### Scenario 4: degraded environments stay fail-open

- GIVEN cozempic is absent from PATH
- WHEN `pre-compact-hook.sh` runs
- THEN it MUST still write the pending marker and the compaction event log line
  and exit 0
- AND GIVEN jq is absent or state files are malformed or the token is
  unresolvable
