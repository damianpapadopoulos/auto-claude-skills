...
- Serena v1.3.0 alignment in `unified-context-stack` skill: surfaced new retrieval tools `find_declaration` and `find_implementations` across the tier doc and four phase docs (triage-and-plan, implementation, testing-and-debug, code-review), and added `mcp__serena__get_diagnostics_for_file` / `_for_symbol` as a Tier 0 fallback when `lsp=false` (testing-and-debug + code-review phases). Session-start banner also names the v1.3.0 retrieval tools. Capability: `unified-context-stack`.
